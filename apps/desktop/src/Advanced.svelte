<script lang="ts">
  ...
  function preset(mode: string) {
    ...
    request.generation.min_words = 2; request.generation.max_words = Math.max(2, request.generation.max_words); request.generation.max_word_length = Math.max(2, request.generation.max_word_length); request.generation.min_word_length = 2; request.generation.min_zipf = 2.7;
  }
</script>
<details class="advanced">
  ...
  <fieldset {disabled}>
    ...
    <label>Ranking workers (0 = automatic)<input type="number" required min="0" max="32" bind:value={request.workers}/></label>
    <label class="check"><input type="checkbox" bind:checked={request.refine}/>Refine word orders (bounded local search)</label>
    <div class="pair">
      <label>Minimum words<input type="number" required min="1" max={generationOnly ? 9007199254740991 : 10} bind:value={request.generation.min_words}/></label>
      <label>Maximum words<input type="number" required min="1" max={generationOnly ? 9007199254740991 : 10} bind:value={request.generation.max_words}/></label>
      <label>Minimum length<input type="number" required min="1" max={relaxed ? 9007199254740991 : 40} bind:value={request.generation.min_word_length}/></label>
      <label>Maximum length<input type="number" required min="1" max={relaxed ? 9007199254740991 : 40} bind:value={request.generation.max_word_length}/></label>
    </div>
    <label>Candidate budget<input type="number" required min={relaxed ? 0 : 1} max={relaxed ? 9007199254740991 : 250000} step="1" bind:value={request.generation.candidate_budget}/></label>
    <div class="pair">
      <label>Deep per group<input type="number" required min="1" max={relaxed ? 9007199254740991 : 10000} bind:value={request.deep_per_group}/></label>
      <label>Results per group<input type="number" required min="1" max={relaxed ? 9007199254740991 : 100} bind:value={request.result_limit_per_group}/></label>
    </div>
    ...
    <label>Minimum frequency (Zipf)<input type="number" required min="0" max={relaxed ? undefined : 10} step="0.1" bind:value={request.generation.min_zipf}/></label>
    ...
    <div class="pair">
      <label>Beam width<input type="number" required min="1" max={relaxed ? 9007199254740991 : 512} bind:value={request.beam_width}/></label>
      <label>Exact up to words<input type="number" required min="1" max="10" bind:value={request.exact_max_words}/></label>
      <label>Retained orders<input type="number" required min="1" max={relaxed ? 9007199254740991 : 128} bind:value={request.retained_orders}/></label>
      <label>Phrase shortlist<input type="number" required min="1" max={relaxed ? 9007199254740991 : 10000} bind:value={request.phrase_rescore_top}/></label>
    </div>
    <label>Maximum phrase bonus<input type="number" required min="0" max={relaxed ? 9007199254740991 : 100} step="0.5" bind:value={request.phrase_bonus_max}/></label>
