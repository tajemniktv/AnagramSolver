# Archived Python solver

This is the retired Python implementation, retained for reference and regression
checks. The current app and CLI are native Rust; see the [project README](../README.md).
Run the commands below from `archive/`. Existing runtime data remains in the
repository's `.anagram_data/`, not a second copy inside this directory. The shared
...
On the first run the solver may download/cache its dictionary, WordNet data, and word-frequency data. Runtime data lives under `../.anagram_data/` at the repository root. The directory is git-ignored. Later runs reuse those caches.
...
## Local native release retention (Windows)

Run `pwsh -File tools/build-release.ps1` to build and smoke-check the native CLI.
On success it retains only `target/release/anagram-cli.exe` and one previous
executable at `target/backup/anagram-cli.exe`, replacing the older backup. An
identical rebuild keeps an existing distinct backup. Build/test caches under
`target` are removed only after verified recovery copies exist in project-local
scratch space; failures retain that scratch directory and report its path.

Use this command after development/testing to return to the two-release layout.
Ordinary Cargo commands can recreate build caches; debug symbols and incremental
compilation are disabled for the default development profile to limit growth.
Deleted compiler artifacts are rebuildable; releases older than the retained
backup are not archived. The release script is for the current development CLI,
not desktop dogfooding or CI. The next app milestone in `PLAN.md` is a Windows
desktop app; website integration is explicitly deferred.
