...
class OrderingLayerTests(unittest.TestCase):
    ...
    def test_prepare_rows_uses_captured_delegate_under_runtime_rebinding(self) -> None:
        rows = [self._row(("c", "a", "b")), self._row(("f", "d", "e"))]
        lex = object()
        with patch.object(rerank, "_CORE_PREPARE_ROWS") as delegate:
            # Reproduce main()'s runtime rebinding. Looking up core.prepare_rows
            # dynamically here would recurse back into rerank.prepare_rows.
